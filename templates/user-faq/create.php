<form method="post" action="<?= url('admin/faq/store') ?>" class="mb-3 mb-lg-5 p-10">
    <div class="inputBox w-100">
        <label class="d-block mt-5"> عنوان سوال </label>
        <input type="text"  name="title" class="w-100 form-control mt-1" value="" placeholder="لطفا عنوان سوال را وارد کنید" />
        <label class="d-block mt-5">  نوع سوال </label>
        <select name="types" class="w-100 form-control mt-1">
            <?php /** @var array $parametrs */
            foreach($parametrs as $faq_type_item) { ?>
                <option  value="<?= $faq_type_item['id'] ?>"><?= $faq_type_item['title'] ?></option>
            <?php } ?>
        </select>
        <label class="d-block mt-5">جواب سوال </label>
        <textarea dir="rtl" name="content" class="editor" id="editor"></textarea>
        <div class=" d-flex align-items-center justify-content-end">
                <button class="btn btn-success mt-5" style="margin-right:auto;" type="submit">افزودن</button>
        </div>
    </div>
</form>
