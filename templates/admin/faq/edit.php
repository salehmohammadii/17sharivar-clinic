...
<form method="post" action="<?= url('admin/faq/update/'.$parametrs['faq']['id']) ?>" class="mb-3 mb-lg-5 p-10">
    <div class="inputBox w-100">
        <label class="d-block mt-5"> عنوان سوال </label>
        <input type="text"  name="title" class="w-100 form-control mt-1" value="<?= $parametrs['faq']['title'] ?>" placeholder="لطفا عنوان سوال را وارد کنید" />
        <label class="d-block mt-5">  نوع سوال </label>
        <select name="types" class="w-100 form-control mt-1">
            <?php /** @var array $parametrs */

            foreach($parametrs['faq_type'] as $faq_type_item) { ?>
                <option <?php if ( $faq_type_item['title']==$parametrs['faq']['ft_title'])echo 'selected'?>  value="<?= $faq_type_item['id'] ?>"><?= $faq_type_item['title'] ?></option>
            <?php } ?>
        </select>
        <label class="d-block mt-5">جواب سوال </label>
        <textarea dir="rtl" name="content" class="editor" id="editor"><?= $parametrs['faq']['content'] ?></textarea>
