...
left join users u on p.user_id = u.id left join skills s on u.skill = s.id ')->fetchAll();
    ...
    public function login()
    {
        ...
        $this->showpage("lab-result.php", "دریافت نتیجه آزمایش",$this->get_general_info());
    }

    public function view_result($request){
      $code=$request['code'];
      $phone=$request['phone'];
      $result=$this->db->select('select * from lab_results where code=? and phone=?',[$code,$phone])->fetch();
      if ($result!=false){
          flash('lab_result_link',$result['link']);
      }else{
          flash('lab_result_error','اطلاعات وارد شده اشتباه است یا هنوز نتیجه آزمایش آپلود نشده است.');
      }
      $this->redirectback();
       }
