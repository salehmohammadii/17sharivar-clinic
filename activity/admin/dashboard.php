...
class dashboard extends admin
{
    public function index(){
       ...
       $messages= $db->select('SELECT COUNT(id) as te from contact')->fetch();
        $new_doctors=$db->select('SELECT users.id,full_name,resume,phone,users.created_at,personalCode,skill,title as sk_title FROM users left join skills on skill=skills.id  where  users.created_at > DATE_SUB(NOW(), INTERVAL 1 DAY)  &&  roll=2')->fetchAll();
